...
# Public artifact-lane residual surface at the low-LR / large-batch boundary.
_BOUNDARY_CALIBRATION = np.array([
    0.0037490447244612393,
    -0.004855067296883465,
    0.008847796328530897,
    0.0014452349974179107,
    0.002138681227749116,
    -0.0017024647504920091,
])
_LOG_LR_ANCHOR = np.log(2.44e-4)
_LOG_BSZ_ANCHOR = np.log(2048.0)
...
def _boundary_correction(log_lr, log_bsz):
    lr_boundary = log_lr - _LOG_LR_ANCHOR
    bsz_boundary = log_bsz - _LOG_BSZ_ANCHOR
    features = np.column_stack([
        np.ones_like(lr_boundary),
        lr_boundary,
        bsz_boundary,
        lr_boundary ** 2,
        bsz_boundary ** 2,
        lr_boundary * bsz_boundary,
    ])
    return features @ _BOUNDARY_CALIBRATION
...
def scaling_law_func(data_points, params):
    ...
    preds = term_sum + bias
    preds = preds + _boundary_correction(log_lr, log_bsz)
...
def fit_scaling_law(
    ...
    *,
    ...
    # ---- residual function -----------------------------------------------
    def _residuals(p):
        log_lr, log_bsz, _, _ = _prepare_features(X)
        pred = scaling_law_func(X, p) - _boundary_correction(log_lr, log_bsz)
        return pred - y
